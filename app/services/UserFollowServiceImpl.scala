...
class UserFollowServiceImpl extends UserFollowService {
  ...
  override def findById(userId: Long)(implicit dBSession: DBSession): Try[List[UserFollow]] = Try {
    UserFollow.where('userId -> userId).apply()
  }

  override def findByFollowId(followId: Long)(implicit dBSession: DBSession): Try[Option[UserFollow]] = Try {
    UserFollow.where('followId -> followId).apply().headOption
  }

  // userIdのユーザーをフォローするユーザーの集合を取得する
  override def findFollowersByUserId(pager: Pager[User],
                                     userId: Long)(implicit dBSession: DBSession): Try[SearchResult[User]] = {
  ...
  override def findFollowingsByUserId(pager: Pager[User],
                                      userId: Long)(implicit dBSession: DBSession): Try[SearchResult[User]] = {
